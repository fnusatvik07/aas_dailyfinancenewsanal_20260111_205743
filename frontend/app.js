...
class AgentChat {
    ...
    async checkBackendStatus() {
        try {
            ...
            const backendUrl = this.backendUrl || (this.backendUrlInput ? this.backendUrlInput.value : null) || 'http://localhost:8002';
            
            console.log('Checking backend health at:', backendUrl + '/health');
            this.updateStatus('🔄 Connecting...', '#ffeaa7');
            
            const response = await fetch(backendUrl + '/health');
            if (response.ok) {
                const data = await response.json();
                this.updateStatus('🟢 Connected', '#55efc4');
                console.log('Backend health check passed:', data);
                this.backendUrl = backendUrl; // Store the working URL
            } else {
                this.updateStatus('❌ Backend Error (' + response.status + ')', '#fd79a8');
                console.error('Backend health check failed:', response.status);
            }
        } catch (error) {
            console.error('Backend connection error:', error);
            this.updateStatus('❌ Connection Failed', '#fd79a8');
    ...
    getBackendUrl() {
        return this.backendUrl || (this.backendUrlInput ? this.backendUrlInput.value : null) || 'http://localhost:8002';
    }
